...
//					return super.next();
					...
					if (result.hasNext()) {
                        if(Configuration.relative_error){
                            TupleExpr te = CalculatePlanQuality.tpQueryMap.get(query);
					        CalculatePlanQuality.addRealCard(te);
					        if(te.getParentNode() instanceof BindJoin && ((BindJoin) te.getParentNode()).getRightArg().equals(te)){
					            CalculatePlanQuality.addRealCard((TupleExpr) te.getParentNode());
                            }
                            if(te.getParentNode() instanceof HashJoin && ((HashJoin) te.getParentNode()).getRightArg().equals(te)) {
                                CalculatePlanQuality.temp = (TupleExpr) te.getParentNode();
                            }
                        }
						last = result.next();
						return last;
