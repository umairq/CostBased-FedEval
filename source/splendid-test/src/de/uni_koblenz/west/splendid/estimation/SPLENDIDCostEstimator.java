...
//			CalculatePlanQuality.estimatedCard.put(join.getLeftArg(),leftCard);
//			CalculatePlanQuality.estimatedCard.put(join.getRightArg(),rightCard);
//			//CalculatePlanQuality.estimatedCard.put(join,this.cost);
//		}
...
//		LOGGER.warn("BindJoin: " + leftCard + " >< " + joinCard + " :: " + (leftCard + joinCard) * C_TRANSFER_TUPLE + 2 * C_TRANSFER_QUERY);
	...
	protected void meet(RemoteQuery query) {
		...
		if(Configuration.relative_error){
		    if(CalculatePlanQuality.estimatedCard.get(query)==null) {
                Double card = cardEst.process(query);
                CalculatePlanQuality.estimatedCard.put(query, card);
            }
		}
	}
