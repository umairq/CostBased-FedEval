...
class evaluateSPARQLQuery {
    ...
    static Boolean relative_error = false;
    ...
    static String relerr_foleder;
    //static int join_number = 0;
    static int tp_number = 1;
    ...
    public static void prepareFedX() {
        String fedxConfig = "/home/MuhammadSaleem/umair/evaluation/odeyssey/roott/federatedOptimizer/lib/fedX3.1/config2";
        String dataConfig = "/home/MuhammadSaleem/umair/evaluation/odeyssey/roott/fedBenchFederation.ttl";
        try {
    ...
    public static void main(String[] args) throws Exception {
        ...
        relative_error = Boolean.parseBoolean(args[8]);
        prepareFedX();
        ...
        if(relative_error)
        	computeRelativeErrors(plans,args[7],args[6],query.toString(),query.getProjectVars().toString());
...
//	        System.out.println(res.next());
    ...
    // Added by Umair
    public static int evaluateSubQuery(String queryStr) {
    	try {
    		queryStr = "SELECT  (COUNT(*) AS ?triples) " +queryStr;
    		TupleQuery query = QueryManager.prepareTupleQuery(queryStr);
    		TupleQueryResult res = query.evaluate();
    		int n = 0;
    		while (res.hasNext()) {
//    	        res.next();
//                n++;  
    		  n+= Long.parseLong(res.next().getValue("triples").stringValue());
                }
    		return n;
    	}catch (Exception e) {
    		e.printStackTrace();
    		System.err.println("Error executting query");
    		return 0;
    	}

    }
    //Added by Umair 
    public static void computeRelativeErrors(HashMap<HashSet<Triple>, Vector<Tree<Pair<Integer,Triple>>>> plans, String error_file_path,String query_name, String query,String projvars) {
		RelativeError.pre_initfileRelativeError(error_file_path,query_name);
		RelativeError.setQuery_name(query_name);
		RelativeError.initializeFile(query_name,query);
		

    	if(plans.keySet().size()==0) {
    		System.out.println("Cardinality is not estimated");
    	}else {
            for (HashSet<Triple> tp : plans.keySet()) {
            	Vector<Tree<Pair<Integer,Triple>>> plan = plans.get(tp);
            	System.out.println(tp.toString());
            	for(Tree<Pair<Integer,Triple>> vTree: plan) {
                    System.out.println("estimated cardinalities: \n"+vTree.getCard());
                    recCardinalityCheck(vTree,plans,projvars,tp,0);
    ...
    static HashMap<Integer,Integer> joinTP= new HashMap<Integer,Integer>();
    //umair
    public static void recCardinalityCheck(Tree<Pair<Integer,Triple>> tree,HashMap<HashSet<Triple>, Vector<Tree<Pair<Integer,Triple>>>> plans, String projectVars, HashSet<Triple> tp2, int j_num) {
        projectVars = projectVars.replaceAll(",", "").replace("[", "").replace("]","");
        long est_card = 0;
        long real_card = 0;
    	if (tree instanceof Leaf<?>) {
            Triple t  = tree.getOneElement().getSecond();
            System.out.println(t.toString() +"\nTriple Estimated Cardinality:"+tree.getCard());
            est_card = tree.getCard();
            Node s = t.getSubject();
            HashSet<Triple> tri = new HashSet<Triple>();
            String str = "";
	          if (s.isURI()) {
	              str += "<"+s.getURI()+"> ";
	          } else {
	          ...
	          tri.add(t);
	          String queryStr = createCountQuery(str+".",projectVars);
            Query countqry = QueryFactory.create(queryStr);
            HashMap<HashSet<Triple>, Vector<Tree<Pair<Integer,Triple>>>> plan_new = new HashMap<HashSet<Triple>, Vector<Tree<Pair<Integer,Triple>>>>();
            Vector<Tree<Pair<Integer,Triple>>> plan_c = new Vector<Tree<Pair<Integer,Triple>>>(plans.get(tp2));
            plan_c.removeAllElements();
            plan_c.add(tree);
            
            plan_new.put(tri, plan_c);
          
            Query newQuery = produceQueryWithServiceClauses(countqry, plan_new);
            if (newQuery != null) {
            	real_card =evaluateSubQuery(newQuery.getQueryPattern().toString());
            	if(j_num==0) {
            		RelativeError.writeTPInfo(est_card, real_card, tp_number);
            		tp_number++;
            	}else {
            		RelativeError.writeJoinTPInfo(est_card, real_card, joinTP.get(j_num));
            		joinTP.put(j_num,joinTP.get(j_num)+1);
            	}
            	SimilarityConstants.realValues.add(real_card);
            	SimilarityConstants.estimatedValues.add(est_card);
            	
            	//System.out.println("Real Cardinality:"+ real_card);
            }
        } else {
            Branch<Pair<Integer, Triple>> b = (Branch<Pair<Integer, Triple>>) tree;
            System.out.println(b.getElements().toString() +"\nJoin Estimated cardinality:"+ b.getCard());
            est_card = b.getCard();
            Set<Pair<Integer, Triple>> elems = b.getElements();
            HashSet<Triple> tri = new HashSet<Triple>();
            String str = "";
            ...
            String queryStr = createCountQuery(str,projectVars.replace(",", projectVars));
            
            Query countqry = QueryFactory.create(queryStr);
//            Query newQuery = produceQueryWithServiceClauses(countqry, plans);
            HashMap<HashSet<Triple>, Vector<Tree<Pair<Integer,Triple>>>> plan_new = new HashMap<HashSet<Triple>, Vector<Tree<Pair<Integer,Triple>>>>();
            Vector<Tree<Pair<Integer,Triple>>> plan_c = new Vector<Tree<Pair<Integer,Triple>>>(plans.get(tp2));
            plan_c.removeAllElements();
            plan_c.add(tree);
            ...
            if (newQuery != null) {
            	real_card = evaluateSubQuery(newQuery.getQueryPattern().toString());
            	RelativeError.writeJoinInfo(est_card, real_card);
            	///join_number++;
            	//System.out.println("Real Cardinality:"+ real_card);
            	SimilarityConstants.realValues.add(real_card);
            	SimilarityConstants.estimatedValues.add(est_card);
            ...
            j_num++;
            if(joinTP.get(j_num)!=null) {
            	j_num = WriteErrorData.join_number;
            }
            joinTP.put(j_num,1);
            Tree<Pair<Integer, Triple>> l = b.getLeft();
            Tree<Pair<Integer, Triple>> r = b.getRight();
            recCardinalityCheck(l,plans,projectVars,tp2,j_num); 
            recCardinalityCheck(r,plans, projectVars,tp2,j_num);   
            j_num=0;
        }
 	
    }
    //added by umair
    public static String createCountQuery(String tp,String projVars) {
    	String strQuery ="";
    	if(projVars.equals("")) {
    	strQuery = "SELECT  (COUNT(*) AS ?triples) " +
