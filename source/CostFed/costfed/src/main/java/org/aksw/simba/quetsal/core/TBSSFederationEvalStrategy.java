...
import org.aksw.simba.quetsal.core.evaluation.*;
...
public class TBSSFederationEvalStrategy extends SparqlFederationEvalStrategy {
	...
	{
		if(expr instanceof QueryRoot && RelativeError.config != null && RelativeError.config.isRelativeError()){
		...
		if (expr instanceof BindJoin) {
			...
			return evaluateLeftJoin((LeftJoin) expr, bindings);
		}
	...
	CloseableIteration<BindingSet, QueryEvaluationException> evaluateLeftJoin(LeftJoin join, BindingSet bindings)
	{
		return new LeftJoinImpl(	this, join, bindings);
	}
