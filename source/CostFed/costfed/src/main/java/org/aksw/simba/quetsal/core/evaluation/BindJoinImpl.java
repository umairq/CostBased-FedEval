...
public class BindJoinImpl extends RestartableLookAheadIteration<BindingSet> {
    ...
    protected final TupleExpr leftArg;
	...
	public void handleClose() {
        ...
        if(queryInfo.getFederation().getConfig().isRelativeError()) {
            for (int i = 0; i < JoinOrderOptimizer.cardJoin.size(); i++) { //childExprs.get(1).toString().contains(JoinOrderOptimizer.estCardPairs2.get(0).expr.toString())
                if (rightArg.toString().contains(JoinOrderOptimizer.cardJoin.get(i).expr2.toString()) &&
                        leftArg.toString().contains(JoinOrderOptimizer.cardJoin.get(i).expr1.toString())) {
...
//
            RelativeError.relativeErrorJoinCard("Bind Join",cardPairsJoin,rightCount);

            QueryEvaluation.idxCount++;
//
        }
