...
//    protected final Config config;
	...
	public class HashJoinTask extends Async<CloseableIteration<BindingSet, QueryEvaluationException>> {
		...
		public void callAsync(CloseableIteration<BindingSet, QueryEvaluationException> res) {
			...
			if(queryInfo.getFederation().getConfig().isRelativeError()) {
				computeRelativeError(idx, gotRecs, putRecs);
			}
			res.close();
		}
	...
	public void computeRelativeError(int idx, int gotRecs,int putRecs){
			List<CardinalityVisitor.CardPair> cardPairs = new ArrayList<>();
			List<CardinalityVisitor.JoinCard> cardPairsJoin = new ArrayList<>();
			int index = 0;
			for(int i = 0; i<JoinOrderOptimizer.estCardPairs2.size(); i++){ //childExprs.get(1).toString().contains(JoinOrderOptimizer.estCardPairs2.get(0).expr.toString())
				if (childExprs.size()>idx&&childExprs.get(idx)!=null&&JoinOrderOptimizer.estCardPairs2.get(i)!=null&&childExprs.get(idx).toString().contains(JoinOrderOptimizer.estCardPairs2.get(i).expr.toString())){
					cardPairs.add(JoinOrderOptimizer.estCardPairs2.get(i));
//					JoinOrderOptimizer.estCardPairs2.remove(i);
					index = i;
				}
			}
			if(JoinOrderOptimizer.estCardPairs2.size()>0)
			    JoinOrderOptimizer.estCardPairs2.remove(index);
			if(idx>0) {
				for (int i = 0; i < JoinOrderOptimizer.cardJoin.size(); i++) { //childExprs.get(1).toString().contains(JoinOrderOptimizer.estCardPairs2.get(0).expr.toString())
					if (childExprs.get(0).toString().contains(JoinOrderOptimizer.cardJoin.get(i).expr1.toString()) &&
							childExprs.get(1).toString().contains(JoinOrderOptimizer.cardJoin.get(i).expr2.toString())) {
						cardPairsJoin.add(JoinOrderOptimizer.cardJoin.get(i));
						//JoinOrderOptimizer.cardJoin.remove(i);
					}
				}
			}

				RelativeError.relativeErrorCardPair(cardPairs,gotRecs);

				if(idx > 0){
					RelativeError.relativeErrorJoinCard("HashJoin",cardPairsJoin,putRecs);
