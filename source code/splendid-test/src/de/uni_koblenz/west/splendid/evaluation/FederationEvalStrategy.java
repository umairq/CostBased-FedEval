...
//		assert join.getNumberOfArguments() > 0;
		if(sources.size()==0)System.out.println("No sources for plan");
...
//			
	...
	public CloseableIteration<BindingSet, QueryEvaluationException> evaluate(
		...
		if(Configuration.relative_error)
            CalculatePlanQuality.addRealJoinText(join);
...
//			Set<Graph> sources = graphMap.get(sp);
	...
	public CloseableIteration<BindingSet, QueryEvaluationException> evaluate(
		...
		if (expr instanceof RemoteQuery) {
            ...
            if(Configuration.relative_error)
                CalculatePlanQuality.saveQueryPlan(expr);
