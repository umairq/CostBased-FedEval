package org.semagrow.semagrow.org.aksw.simba.quetsal.util;
...
//			 , "http://" + host + ":8889/sparql"
	...
	//	String namedGraph = "http://aksw.org/fedbench/";  //can be null. in that case all graph will be considered 
        ...
        if(args.length>0){
            endpointPath = args[0];
            ep = new EndpointProvider(endpointPath);
	 ...
	 * graph Named graph. Can be null. In this case all named graphs will be considered for Quetzal summaries
	 *  branchLimit Branching limit
	...
	public void generateSummaries(List<String> endpoints) throws IOException
	{
		...
		final AtomicInteger dsnum = new AtomicInteger(0);
